...
e retorna a que tenha a menor quantidade de elementos
*/
/*
#include <iostream>
#include <vector>
...
void soma_sublista_bt(int* a, int tam, int soma, vector<int>& atual, vector<int>& menor) {
    if (soma == 0) {
        if (menor.empty() || atual.size() < menor.size())
            menor = atual;
        return;
    }
    if (soma < 0 || tam == 0)
    ...
    soma_sublista_bt(a, tam - 1, soma - a[tam - 1], atual, menor);
    atual.pop_back();

    soma_sublista_bt(a, tam - 1, soma, atual, menor);
}
...
void exibirMenorSublista(const vector<int>& arr, int s) {
    int n = arr.size();
    vector<int> melhor_sublista;
    int menor_tamanho = n + 1;
